...
app.post("/uploads", (req, res) => {
  parseMultipart(req, res, (filePath) => {
    ...
    const finalPath = `${process.env.API}/uploads/${path.basename(filePath)}` 

    /* {
    id: 190,
    image: "localhost:3002/uploads/src\\uploads\\6aMSB6LYjrCxrxiwb.jpeg",
  },
  */
    ...
    // STEP 3: Writing to a file 0
    fs.writeFile("./src/data/product.json", JSON.stringify(
  ...
  req.on("end", () => {
    if (fileData) {
      const filePath = path.join(
        uploadDir,
        generateRandomId() + path.extname(fileName)
      );
...
function RandomId() {
  return Math.floor(Math.random() * 100 * Math.PI);
}
